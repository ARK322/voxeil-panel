...
import type { RestoreDbInput, RestoreFilesInput } from "./backup.dto.js";
...
async function resolveBackupName(
  dir: string,
...
async function waitForPodCompletion(namespace: string, name: string): Promise<void> {
...
async function deletePodSafely(namespace: string, name: string): Promise<void> {
  const { core } = getClients();
  try {
...
async function runDbRestore(archivePath: string, dbName: string): Promise<void> {
  const { host, user, password, port } = requireDbConfig();
  await fs.access(archivePath).catch((error: any) => {
...
export async function restoreSiteFiles(
  slug: string,
  input: RestoreFilesInput
): Promise<{ ok: true; slug: string; restored: "files"; archive: string }> {
  ...
  await fs.access(path.join(dir, archive)).catch((error: any) => {
    if (error?.code === "ENOENT") {
      throw new HttpError(404, "Backup archive not found.");
    }
    throw error;
...
export async function restoreSiteDb(
  slug: string,
  input: RestoreDbInput
): Promise<{ ok: true; slug: string; restored: "db"; archive: string }> {
  const normalized = normalizeSlug(slug);
