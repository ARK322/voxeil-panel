...
function requireAdminConfig(): AdminConfig {
  ...
  const password =
    process.env.POSTGRES_ADMIN_PASSWORD?.trim() ?? process.env.DB_ADMIN_PASSWORD?.trim();
...
function normalizePrefix(value: string, label: string): string {
  const normalized = value.trim().toLowerCase();
  if (!/^[a-z0-9_]*$/.test(normalized)) {
...
export function normalizeSlugIdentifier(slug: string): string {
  const normalized = slug.trim().toLowerCase().replace(/-/g, "_");
  return normalizeIdentifier(normalized, "slug");
}

export function normalizeDbName(value: string): string {
  return normalizeIdentifier(value, "dbName");
}

export function normalizeDbUser(value: string): string {
...
export async function ensureRole(username: string, password: string): Promise<{ created: boolean }> {
...
export async function ensureDatabase(
  ...
  return withAdminClient(async (client) => {
    ...
    await client.query(
      `GRANT ALL PRIVILEGES ON DATABASE ${quoteIdent(safeDb)} TO ${quoteIdent(safeOwner)}`
    );
    ...
    for (const row of dbList.rows as Array<{ datname: string }>) {
      if (row.datname === safeDb) continue;
      await client.query(
        `REVOKE CONNECT ON DATABASE ${quoteIdent(row.datname)} FROM ${quoteIdent(safeOwner)}`
      );
    }
...
export async function revokeAndTerminate(dbName: string): Promise<void> {
  const safeDb = normalizeDbName(dbName);
  await withAdminClient(async (client) => {
    const dbResult = await client.query("SELECT 1 FROM pg_database WHERE datname = $1", [safeDb]);
    if ((dbResult.rowCount ?? 0) === 0) return;
...
export async function dropRole(username: string): Promise<void> {
  const safeUser = normalizeDbUser(username);
  await withAdminClient(async (client) => {
