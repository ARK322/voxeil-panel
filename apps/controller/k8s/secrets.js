...
async function readPlatformSecret(name) {
    ...
    try {
        ...
        if (error?.response?.statusCode === 404) {
            throw new HttpError(500, `Missing ${name} secret in ${PLATFORM_NAMESPACE} namespace.`);
        }
        throw error;
    }
}
export async function ensureGhcrPullSecret(namespace) {
    const { core } = getClients();
    let source;
    try {
