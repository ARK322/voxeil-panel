...
export const USER_BACKUP_PVC_NAME = "pvc-user-backup";
...
const DEFAULT_USER_BACKUP_SIZE_GI = 50; // Default 50Gi for user backup
...
export async function ensureUserBackupPvc(namespace, sizeGi = DEFAULT_USER_BACKUP_SIZE_GI) {
    const { core } = getClients();
    try {
        await core.createNamespacedPersistentVolumeClaim(namespace, {
            metadata: { 
                name: USER_BACKUP_PVC_NAME,
                labels: {
                    "voxeil.io/pvc-type": "user-backup"
                }
            },
            spec: {
                accessModes: ["ReadWriteOnce"],
...
export async function getUserBackupPvcSizeGi(namespace) {
    const { core } = getClients();
    try {
        const pvc = await core.readNamespacedPersistentVolumeClaim(USER_BACKUP_PVC_NAME, namespace);
        const value = pvc.body.spec?.resources?.requests?.storage;
        return parseGiToNumber(value);
    }
    catch (error) {
